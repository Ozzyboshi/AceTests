...
void memtestGsLoop(void) {
  ...
  static const ULONG ulSize = 100000;
  UBYTE* pBuffer = memAlloc(ulSize,MEMF_CHIP);
  memset(pBuffer,0xFF,ulSize);
  #ifdef PLEASEFREE
  memFree(pBuffer,ulSize);
  #endif

  UBYTE* pBuffer2 = AllocMem(ulSize,MEMF_CHIP);
  memset(pBuffer2,0xFF,ulSize);
  #ifdef PLEASEFREE
  FreeMem(pBuffer2,ulSize);
  #endif
  
  vPortWaitForEnd(s_pVpMain);
}

void memtestGsDestroy(void) {
